...
from repanier.xlsx.xslx_stock import export_stock


def export_invoice(permanence, customer=None, producer=None, wb=None, sheet_name=""):
    ws = None
    ...
    for purchase in purchase_set:
        ...
        if hide_customer_prices:
            ...
            if repanier.apps.REPANIER_SETTINGS_DISPLAY_VAT:
                row += [
        ...
        if repanier.apps.REPANIER_SETTINGS_DISPLAY_VAT or not hide_producer_prices:
            row += [
    ...
    if wb is not None:
        ...
        if hide_producer_prices:
            ...
            if not repanier.apps.REPANIER_SETTINGS_DISPLAY_VAT:
...
def admin_export(request, queryset):
    ...
    wb = export_invoice(permanence=permanence, sheet_name=current_site.name)
