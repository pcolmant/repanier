...
def import_xslx_view(admin_ui, admin, request, queryset, handle_uploaded_file, action):
    if 'apply' in request.POST:
        ...
        admin_ui.message_user(request, _("Action canceled by the user."), level=messages.WARNING)
        return HttpResponseRedirect(request.get_full_path())
    form = ImportXlsxForm(
