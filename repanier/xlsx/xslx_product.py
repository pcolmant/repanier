...
def export(producer, wb=None, producer_prices=True):
    ...
    now = timezone.now()
    wb, ws = new_landscape_a4_sheet(wb, producer.short_profile_name, now.strftime('%d-%m-%Y %H:%M'))
    ...
    for product in product_set:
        row = [
            ...
            # Need to have same column name into export and import sheet
            (_("producer unit price") , 10,
            # (_("producer unit price") if producer_prices else _("customer unit price") , 10,
