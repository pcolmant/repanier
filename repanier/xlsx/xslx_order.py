...
from repanier.models import Customer, PurchaseOpenedOrClosed, OfferItem, CustomerInvoice, ProducerInvoice
from repanier.models import Permanence
from repanier.models import PermanenceBoard
from repanier.models import Producer
from repanier.models import Staff
from repanier.tools import get_base_unit
...
def export_abstract(permanence, delivery_point=None, wb=None):
    ...
    if permanence is not None:
        ...
        wb, ws = new_portrait_a4_sheet(wb, permanence, '', header=header, add_print_title=False)
        row_num = 1
        # Customer info
        customer_set = Customer.objects.filter(
            purchase__permanence_id=permanence.id, represent_this_buyinggroup=False)
        if delivery_point is not None:
            customer_set = customer_set.filter(delivery_point=delivery_point)
...
def export_customer_label(permanence, delivery_point=None, wb=None):
    # Customer label
    # if wb is None:
    #     wb = Workbook()
...
def export_preparation(permanence, delivery_point=None, wb=None):
    yellowFill = Fill()
    ...
    if delivery_point is not None:
        producer_set = producer_set.filter(purchase__customer__delivery_point=delivery_point)
    ...
    if producer is not None:
        ...
        while producer is not None:
            ...
            if producer.invoice_by_basket:
                purchase_set = PurchaseOpenedOrClosed.objects.filter(
                ...
                if delivery_point is not None:
                    purchase_set = purchase_set.filter(customer__delivery_point=delivery_point)
...
def export_producer_by_product(permanence, producer, delivery_point=None, wb=None):
    yellowFill = Fill()
    yellowFill.start_color.index = 'FFEEEE11'
    yellowFill.end_color.index = 'FFEEEE11'
    yellowFill.fill_type = Fill.FILL_SOLID
    ...
    if offer_item:
                ...
                if offer_item.wrapped: # or offer_item.limit_order_quantity_to_stock:
                    ...
                    for purchase in purchase_set:
                        ...
                        if first_purchase:
                            ...
                            c.style.font.color.index = 'FF939393'
                        c = ws.cell(row=row_num, column=4)
                        if not offer_item.producer_pre_opening:
                    ...
                    if not offer_item.producer_pre_opening:
                        ...
                        # Use customer price
                        c.value = offer_item.customer_unit_price
                    c.style.number_format.format_code = '_ € * #,##0.00_ ;_ € * -#,##0.00_ ;_ € * "-"??_ ;_ @_ '
                    c.style.borders.bottom.border_style = Border.BORDER_THIN
                    c = ws.cell(row=row_num, column=5)
                    c.value = offer_item.unit_deposit
        ...
        for col_num in xrange(7):
            ...
            c.style.borders.bottom.border_style = Border.BORDER_THIN
            if col_num == 1:
                c.value = "%s %s" % (_("Total Price"), repanier.apps.REPANIER_SETTINGS_GROUP_NAME)
                c.style.number_format.format_code = NumberFormat.FORMAT_TEXT
            if col_num == 6:
...
def export_producer_by_customer(permanence, producer, delivery_point=None, wb=None):
    translation.activate(producer.language)
        ...
        while (purchase is not None):
            ...
            while (purchase is not None and customer_save.id == purchase.customer_id):
                ...
                if first_purchase:
                    ...
                    first_purchase = False
                c = ws.cell(row=row_num, column=3)
                if not producer.producer_pre_opening:
                    c.value = purchase.get_producer_unit_price()
                else:
                    c.value = purchase.get_customer_unit_price()
                c.style.number_format.format_code = '_ € * #,##0.00_ ;_ € * -#,##0.00_ ;_ € * "-"??_ ;_ @_ '
...
def export_customer(permanence, customer=None, wb=None, ws_preparation_title=None, delivery_point=None, deposit=False):
    yellowFill = Fill()
    yellowFill.start_color.index = 'FFEEEE11'
    yellowFill.end_color.index = 'FFEEEE11'
    yellowFill.fill_type = Fill.FILL_SOLID
    ...
    if purchase:
        ...
        while (purchase is not None):
            ...
            first_purchase = True
            while purchase is not None and customer_save.id == purchase.customer_id:
                department_for_customer_save__id = purchase.offer_item.department_for_customer_id
                department_for_customer_save__short_name = purchase.offer_item.department_for_customer.short_name \
                                if purchase.offer_item.department_for_customer is not None else "---"
                while purchase is not None and customer_save.id == purchase.customer_id and \
                    purchase.offer_item.department_for_customer_id == department_for_customer_save__id:
            ...
            for col_num in xrange(10):
                ...
                c.style.borders.bottom.border_style = Border.BORDER_MEDIUMDASHED
            row_num += 2
            if deposit and repanier.apps.REPANIER_SETTINGS_PAGE_BREAK_ON_CUSTOMER_CHECK:
...
def admin_customer_export(request, queryset):
    ...
    if permanence is not None:
        ...
        wb = export_abstract(permanence=permanence, wb=None)
        if wb is not None:
