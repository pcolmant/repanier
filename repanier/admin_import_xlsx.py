...
def handle_product_uploaded_file(request, queryset, file_to_import):
	...
	for producer in queryset:
		import_producer_products(wb.get_sheet_by_name(producer.short_profile_name), producer=producer, product_reorder=product_reorder)
	order = 0
	...
	if 'apply' in request.POST:
		...
		if form.is_valid():
			...
			if('.xlsx' in file_to_import.name) and (file_to_import.size <= 1000000):
				handle_product_uploaded_file(request, queryset, file_to_import)
			...
			producer.message_user(request, _("Error when importing %s.") % (file_to_import.name))
...
def handle_permanence_done_uploaded_file(request, queryset, file_to_import):
	...
	for permanence in queryset:
		...
		if ws:
			import_permanence_purchases(ws, permanence=permanence)
	ws = wb.get_sheet_by_name(_("bank movements"))
	if ws:
		import_bank_movements(ws)
	return
...
def import_permanence_done_xlsx(permanence, admin, request, queryset):
	...
	if 'apply' in request.POST:
		...
		if form.is_valid():
			file_to_import = request.FILES['file_to_import']
			if('.xlsx' in file_to_import.name) and (file_to_import.size <= 1000000):
				handle_permanence_done_uploaded_file(request, queryset, file_to_import)
			...
			permanence.message_user(request, _("Error when importing %s.") % (file_to_import.name))
		return HttpResponseRedirect(request.get_full_path())
	if not form:
		form = ImportXlsxForm(
			initial={'_selected_action': request.POST.getlist(admin.ACTION_CHECKBOX_NAME)}
