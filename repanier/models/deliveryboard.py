...
class DeliveryBoard(models.Model):
    ...
    def set_status(self, new_status):
        ...
        CustomerInvoice.objects.filter(delivery_id=self.id).update(status=new_status)
        PurchaseWoReceiver.objects.filter(customer_invoice__delivery_id=self.id).update(
            status=new_status
        )
