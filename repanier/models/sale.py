...
class Sale(TranslatableModel):
    ...
    sale_date = models.DateField(_("Date"), db_index=True)
    ...
    master_sale = models.ForeignKey(
        "Sale",
        verbose_name=_("Master sale"),
        related_name="child_sale",
        blank=True,
        null=True,
        default=None,
    ...
    # canceled_invoice_sort_order is used in admin.sale_done to display the canceled sale in correct order
    ...
    def get_html_producers(self, with_download):
        ...
        if self.status == SALE_PLANNED:
            download_url = add_filter(
                reverse("admin:sale-export-offer", args=[self.id])
            )
            ...
            download_url = add_filter(
                reverse("admin:sale-export-producer-opened-order", args=[self.id])
            )
            button_download = format_html(
                '<span class="repanier-a-container"><a class="repanier-a-tooltip repanier-a-info" href="{}" data-repanier-tooltip="{}"><i class="fas fa-download"></i></a></span>',
                download_url,
            ...
            for p in self.producers.all().only("id"):
                pi = (
                    ProducerInvoice.objects.filter(producer_id=p.id, sale_id=self.id)
                    .order_by("?")
                    .first()
                )
                if pi is not None:
            ...
            if self.status == SALE_SEND:
                download_url = add_filter(
                    reverse("admin:sale-export-producer-closed-order", args=[self.id])
                )
                button_download = format_html(
                    '<span class="repanier-a-container"><a class="repanier-a-tooltip repanier-a-info" href="{}" data-repanier-tooltip="{}"><i class="fas fa-download"></i></a></span>',
                    download_url,
            ...
            for pi in (
                ProducerInvoice.objects.filter(sale_id=self.id)
            ...
            producers = [
                p.short_name
                for p in Producer.objects.filter(producerinvoice__sale_id=self.id).only(
                    "short_name"
                )
            ]
        if len(producers) > 0:
    ...
    def get_html_customers(self, with_download):
        ...
        if self.status in [SALE_OPENED, SALE_SEND]:
            ...
            if self.status == SALE_OPENED:
                download_url = add_filter(
                    reverse("admin:sale-export-customer-opened-order", args=[self.id])
                )
            else:
                download_url = add_filter(
                    reverse("admin:sale-export-customer-closed-order", args=[self.id])
                )
            button_download = format_html(
                '<span class="repanier-a-container"><a class="repanier-a-tooltip repanier-a-info" href="{}" data-repanier-tooltip="{}"><i class="fas fa-download"></i></a></span> ',
                download_url,
            ...
            delivery_save = None
            for ci in (
                CustomerInvoice.objects.filter(sale_id=self.id)
            ...
            customers = [
                c.short_name
                for c in Customer.objects.filter(customerinvoice__sale_id=self.id).only(
                    "short_name"
                )
            ]
        if len(customers) > 0:
    ...
    def get_html_board(self):
        ...
        if sa_set:
            for sa in sa_set:
                ...
                first_board = False
        if not first_board:
            # At least one role is defined in the sale activities
            msg_html = """
    ...
    def set_status(
        ...
        if self.with_delivery_point:
            qs = (
                DeliveryBoard.objects.filter(sale_id=self.id)
                .exclude(status=new_status)
                .order_by("?")
            )
            if not everything:
            ...
            if everything:
                ProducerInvoice.objects.filter(sale_id=self.id).order_by("?").update(
                    status=new_status
                )
        else:
            from repanier.models.purchase import PurchaseWoReceiver

            PurchaseWoReceiver.objects.filter(sale_id=self.id).exclude(
                status=new_status
            ).order_by("?").update(status=new_status)
            CustomerInvoice.objects.filter(sale_id=self.id).order_by("?").update(
                status=new_status
            )
            ProducerInvoice.objects.filter(sale_id=self.id).order_by("?").update(
                status=new_status
        ...
        if everything:
            ...
            sale.is_updated_on = self.is_updated_on = now
            sale.status = self.status = new_status
            if self.highest_status < new_status:
                sale.highest_status = self.highest_status = new_status
            if update_payment_date:
                if payment_date is None:
                    sale.payment_date = self.payment_date = now
                else:
                    sale.payment_date = self.payment_date = payment_date
        # Unlock sale
        sale.save()
    ...
    def back_to_scheduled(self):
        ...
        for offer_item in (
            OfferItemWoReceiver.objects.filter(sale_id=self.id, may_order=True)
            .order_by("producer_id")
            .distinct("producer_id")
        ):
            self.producers.add(offer_item.producer_id)
        OfferItemWoReceiver.objects.filter(sale_id=self.id).update(may_order=False)
    ...
    def close_order(self, everything, deliveries_id=(), send_mail=True):
        ...
        if settings.REPANIER_SETTINGS_CUSTOMER_MUST_CONFIRM_ORDER:
            ...
            customer_invoice_qs = CustomerInvoice.objects.filter(
                sale_id=self.id, is_order_confirm_send=False, status=self.status
            ).order_by("?")
            if self.with_delivery_point:
            ...
            if (
                ...
                customer_invoice_qs = (
                    CustomerInvoice.objects.filter(
                        sale_id=self.id, customer_charged_id=F("customer_id")
                    )
                    .select_related("customer")
                    .order_by("?")
                )
                ...
                for customer_invoice in customer_invoice_qs:
                    ...
                    if not customer.is_default:
                        ...
                        if customer.membership_fee_valid_until < self.sale_date:
        ...
        if everything or self.with_delivery_point:
            ...
            customer_qs = Customer.objects.filter(
                may_order=True,
                customerinvoice__sale_id=self.id,
                is_default=False,
            ).order_by("?")
            if self.with_delivery_point:
            ...
            for customer in customer_qs:
                offer_item_qs = OfferItem.objects.filter(
                    sale_id=self.id, order_unit=PRODUCT_ORDER_UNIT_DEPOSIT
                ).order_by("?")
                # if not everything:
    ...
    def invoice(self, payment_date):
        ...
        default_customer_invoice = CustomerInvoice.get_or_create(
            sale_id=self.id, customer_id=default_customer.id
        )
        default_customer_invoice.set_next_balance(payment_date)
        default_customer_invoice.save()

        if ProducerInvoice.objects.do_not_invoice(sale_id=self.id).exists():
            # Move the producers not invoiced into a new sale
            ...
            customers_to_move = list(
                CustomerProducerInvoice.objects.filter(
                    sale_id=self.id, producer_id__in=producers_to_move
                ).values_list("customer_id", flat=True)
            )
            new_sale = self.create_child()
            new_sale.producers.add(*producers_to_move)
            ProducerInvoice.objects.filter(
                sale_id=self.id, producer_id__in=producers_to_move
            ).update(sale_id=new_sale.id, status=SALE_SEND)
            CustomerProducerInvoice.objects.filter(
                sale_id=self.id,
                producer_id__in=producers_to_move
                # Redundant : customer_id__in=customers_to_move
            ).order_by("?").update(sale_id=new_sale.id)
            OfferItemWoReceiver.objects.filter(
                sale_id=self.id, producer_id__in=producers_to_move
            ).order_by("?").update(sale_id=new_sale.id)

            for old_customer_invoice in CustomerInvoice.objects.filter(
                sale_id=self.id, customer_id__in=customers_to_move
            ).order_by("?"):
            ...
            new_sale.set_status(old_status=SALE_WAIT_FOR_INVOICED, new_status=SALE_SEND)
            new_sale.recalculate_order_amount(re_init=True)
            new_sale.save()

        for customer_invoice in CustomerInvoice.objects.filter(
            sale_id=self.id, customer_id=F("customer_charged_id")
        ).order_by("?"):
        ...
        for bank_account in (
            ...
            CustomerInvoice.get_or_create(
                sale_id=self.id, customer_id=bank_account.customer_id
            )
        for customer_invoice in CustomerInvoice.objects.filter(
            sale_id=self.id
        ).order_by("?"):
        ...
        for bank_account in (
            ...
            ProducerInvoice.get_or_create(
                sale_id=self.id, producer_id=bank_account.producer_id
            )
        ...
        # Important : Create a new bank total for this sale even if there is no bank movement
        ...
        Sale.objects.filter(id=self.id).order_by("?").update(
    ...
    def cancel_invoice(self, last_bank_account_total):
        ...
        ProducerInvoice.objects.filter(
            sale_id=self.id, producer__is_default=False
        ).order_by("?").update(
            bank_amount_in=DECIMAL_ZERO,
            bank_amount_out=DECIMAL_ZERO,
        ...
        ProducerInvoice.objects.filter(
            sale_id=self.id, producer__is_default=True
        ).order_by("?").update(
            bank_amount_in=DECIMAL_ZERO,
            bank_amount_out=DECIMAL_ZERO,
            balance=F("previous_balance"),
    ...
    def cancel_delivery(self):
        ...
        bank_account = BankAccount.get_closest_to(self.sale_date)
        if bank_account is not None:
    ...
    def duplicate(self, dates):
        ...
        for date in dates[:56]:
            ...
            if not same_exists:
                creation_counter += 1
                new_sale = Sale.objects.create(sale_date=date)
                self.duplicate_short_name(new_sale, cur_language)
                self.duplicate_sale_board(new_sale)
                self.duplicate_delivery_board(new_sale, cur_language)
                self.duplicate_producers(new_sale)
        return creation_counter

    def create_child(self):
        new_child_sale = Sale.objects.create(
            sale_date=self.sale_date,
            master_sale_id=self.id,
            status=self.status,
        )
        cur_language = translation.get_language()
        self.duplicate_short_name(new_child_sale, cur_language)
        self.duplicate_sale_board_and_registration(new_child_sale)
        self.duplicate_delivery_board(new_child_sale, cur_language)
        return new_child_sale
    ...
    def check_if_same_exists(self, date, short_name, cur_language):
        if short_name != EMPTY_STRING:
            ...
            same_exists = False
            for existing_sale in Sale.objects.filter(sale_date=date).order_by("?"):
                try:
                    short_name = existing_sale.short_name
    ...
    def duplicate_short_name(self, new_sale, cur_language):
        for language in settings.PARLER_LANGUAGES[settings.SITE_ID]:
            language_code = language["code"]
            translation.activate(language_code)
            new_sale.set_current_language(language_code)
            self.set_current_language(language_code)
            try:
                new_sale.short_name = self.safe_translation_getter(
                    "short_name", any_language=True
                )
                new_sale.save_translations()
            except TranslationDoesNotExist:
                pass
        translation.activate(cur_language)

    def duplicate_producers(self, new_sale):
        for a_producer in self.producers.all():
            new_sale.producers.add(a_producer)

    def duplicate_delivery_board(self, new_sale, cur_language):
        for delivery_board in DeliveryBoard.objects.filter(sale=self).order_by("?"):
            new_delivery_board = DeliveryBoard.objects.create(
                sale=new_sale, delivery_point=delivery_board.delivery_point
            )
            for language in settings.PARLER_LANGUAGES[settings.SITE_ID]:
                language_code = language["code"]
                translation.activate(language_code)
    ...
    def duplicate_sale_board_and_registration(self, new_sale):
        for sale_board in SaleActivity.objects.filter(sale=self).order_by("?"):
            SaleActivity.objects.create(
                ...
                customer=sale_board.customer,
                is_registered_on=sale_board.is_registered_on,
            )

    def generate_bank_payment(self, payment_date: datetime):

        for producer_invoice in ProducerInvoice.objects.filter(
            sale_id=self.id, invoice_sort_order__isnull=True
        ).select_related("producer"):
            ...
            if producer_invoice.balance_invoiced.amount > DECIMAL_ZERO:
                ...
                if producer_invoice.reference:
                    ...
                    if producer.is_default:
                        operation_comment = self.get_sale_display()
                    else:
                        operation_comment = _(
                            "Delivery %(current_site)s - %(sale)s. Thanks!"
                        ) % {
                            "current_site": settings.REPANIER_SETTINGS_GROUP_NAME,
                            "sale": self.get_sale_display(),
                        }
    ...
    def set_qty_invoiced(self):
        ...
        for offer_item in OfferItem.objects.filter(
            sale_id=self.id, order_unit=PRODUCT_ORDER_UNIT_PC_KG
    ...
    def recalculate_order_amount(self, offer_item_qs=None, re_init=False):
        ...
        if re_init:
            ...
            CustomerProducerInvoice.objects.filter(sale_id=self.id).order_by(
                "?"
            ).update(
                total_purchase_with_tax=DECIMAL_ZERO,
                total_selling_with_tax=DECIMAL_ZERO,
            )
            OfferItemWoReceiver.objects.filter(sale_id=self.id).order_by("?").update(
                qty=DECIMAL_ZERO,
                total_purchase_with_tax=DECIMAL_ZERO,
                total_selling_with_tax=DECIMAL_ZERO,
            )

        purchase_set = Purchase.objects.filter(sale_id=self.id).order_by("?")
    ...
    def reorder_purchases(self):
        ...
        Purchase.objects.filter(sale_id=self.id).update(
    ...
    def reorder_offer_items(self):
        ...
        cur_language = translation.get_language()
        offer_item_qs = OfferItemWoReceiver.objects.filter(sale_id=self.id).order_by(
            "?"
        )
        for language in settings.PARLER_LANGUAGES[settings.SITE_ID]:
            language_code = language["code"]
    ...
    def get_html_status_display(self, force_refresh=True):
        ...
        if self.with_delivery_point and self.status < SALE_INVOICED:
            ...
            status = None
            status_counter = 0
            for delivery in DeliveryBoard.objects.filter(sale_id=self.id).order_by(
                "status", "id"
            ):
    ...
    def get_sale_display(self):
        short_name = self.safe_translation_getter("short_name", any_language=True)
        if short_name:
            sale_display = "{}".format(short_name)
        else:
            from repanier.globals import REPANIER_SETTINGS_SALE_ON_NAME

            sale_display = "{}{}".format(
                REPANIER_SETTINGS_SALE_ON_NAME,
                self.sale_date.strftime(settings.DJANGO_SETTINGS_DATE),
            )
        return sale_display

    def get_sale_admin_display(self):
        return self.get_sale_display()

    get_sale_admin_display.short_description = _("Offers")

    def get_html_sale_title_display(self):
        return self.get_html_sale_display(align="vertical-align: bottom; ")

    def get_html_sale_display(self, align=EMPTY_STRING):
        if settings.REPANIER_SETTINGS_TEMPLATE == "bs3":
            if self.status == SALE_OPENED:
                return "{} - {}".format(
                    self.get_sale_display(), self.get_status_display()
                )
            else:
                return "{} - {}".format(self.get_sale_display(), _("Orders closed"))
        else:
            if self.status == SALE_OPENED:
                return mark_safe(
                    '<span class="fa fa-unlock" style="{}color:#cdff60"></span> {}'.format(
                        align, self.get_sale_display()
                    )
                )
            else:
                return mark_safe(
    ...
    def get_html_sale_card_display(self):
        if settings.REPANIER_SETTINGS_TEMPLATE == "bs3":
            ...
            """.format(
                    ...
                    title=self.get_html_sale_display(),
    ...
    class Meta:
        ...
        index_together = [["sale_date"]]


class SaleInPreparation(Sale):
    class Meta:
        proxy = True
