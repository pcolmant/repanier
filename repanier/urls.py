...
if django.VERSION[0] < 2:
    ...
    urlpatterns = [
        ...
        #     name='basket_view'),
        url(
            r"^like/(?P<permanence_id>\d+)/$",
            never_cache(OrderView.as_view()),
            {"like": True},
            name="like_view",
        ),
        url(
            r"^order-filter/(?P<permanence_id>\d+)/$",
            order_filter_view,
            name="order_filter_view",
        ),
        url(
            r"^order-description/(?P<permanence_id>\d+)/$",
        ...
        url(
            r"^ajax/my-cart_amount/(?P<permanence_id>\d+)/$",
            my_cart_amount_ajax,
        ...
        url(
            r"^ajax/btn-confirm-order/$",
            btn_confirm_order_ajax,
            name="btn_confirm_order_ajax",
        ),
        url(
            r"^ajax/display-status/(?P<permanence_id>\d+)/$",
            display_status,
            name="display_status",
        ),
        ...
        url(
            r"^ajax/test-mail-config/$",
            test_mail_config_ajax,
            name="test_mail_config_ajax",
        ),
        url(
            r"^permanence/$",
            never_cache(PermanenceView.as_view()),
            name="permanence_view",
        ),
        url(
            r"^customer-invoice/(?P<pk>[0-9]+)/$",
            login_required(CustomerInvoiceView.as_view()),
            name="customer_invoice_view",
        ),
        url(
            r"^producer-invoice/(?P<pk>[0-9]+)/(?P<uuid>[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12})/$",
        ...
        url(
            r"^rest/permanence/(?P<permanence_id>\d+)/(?P<producer_name>.*)/(?P<reference>.*)/$",
            permanence_producer_product_rest,
            name="permanence_producer_product_rest",
        ),
        url(
            r"^rest/permanence/(?P<permanence_id>\d+)/(?P<producer_name>.*)/$",
            permanence_producer_rest,
            name="permanence_producer_rest",
        ),
        url(
            r"^rest/departments-for-customers/$",
            departments_for_customers_rest,
            name="departments_for_customers_rest",
        ),
        url(
            r"^rest/department-for-customer/(?P<short_name>.*)/$",
            department_for_customer_rest,
            name="department_for_customer_rest",
        ),
        url(r"^rest/producers/$", producers_list, name="producers_rest"),
        url(
            r"^rest/producer/(?P<short_profile_name>.*)/$",
            producer_detail,
            name="producer_rest",
        ),
        url(
            r"^rest/products/(?P<producer_short_profile_name>.*)/$",
            products_rest,
            name="products_rest",
        ),
        url(
        ...
        url(
            r"^unsubscribe/(?P<customer_id>.*)/(?P<token>[\w.:\-_=]+)/$",
            unsubscribe_view,
    ...
    urlpatterns = [
        ...
        path(
            "ajax/delivery-select/", delivery_select_ajax, name="delivery_select_ajax"
        ),
        ...
        # url(r'^ajax/test-mail-config/(?P<id_email_host>.*)/(?P<id_email_port>.*)/(?P<id_email_use_tls>.*)/(?P<id_email_host_user>.*)/(?P<id_new_email_host_password>.*)/$', test_mail_config_ajax, name='test_mail_config_ajax'),
        path(
            "ajax/test-mail-config/",
            test_mail_config_ajax,
            name="test_mail_config_ajax",
        ),
        path(
            "permanence/", never_cache(PermanenceView.as_view()), name="permanence_view"
        ),
        path(
        ...
        path(
            "rest/producer/<str:short_profile_name>/",
            producer_detail,
            name="producer_rest",
        ),
        path(
