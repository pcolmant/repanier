    ...
    if product.producer.producer_price_are_wo_vat:
        ...
        if not product.producer.is_resale_price_fixed:
            if product.order_unit < PRODUCT_ORDER_UNIT_DEPOSIT:
                ...
                product.producer_unit_price * product.producer.price_list_multiplier).quantize(
                    TWO_DECIMALS)
            else:
        ...
        if product.vat_level == VAT_400:
            product.producer_vat = product.producer_unit_price - (product.producer_unit_price / DECIMAL_1_06).quantize(
                FOUR_DECIMALS)
        elif product.vat_level == VAT_500:
            product.producer_vat = product.producer_unit_price - (product.producer_unit_price / DECIMAL_1_12).quantize(
                FOUR_DECIMALS)
        elif product.vat_level == VAT_600:
            product.producer_vat = product.producer_unit_price - (product.producer_unit_price / DECIMAL_1_21).quantize(
                FOUR_DECIMALS)
        ...
        product.customer_vat = DECIMAL_ZERO
        if product.vat_level == VAT_400:
            product.customer_vat = product.customer_unit_price - (product.customer_unit_price / DECIMAL_1_06).quantize(
                FOUR_DECIMALS)
        elif product.vat_level == VAT_500:
            product.customer_vat = product.customer_unit_price - (product.customer_unit_price / DECIMAL_1_12).quantize(
                FOUR_DECIMALS)
        elif product.vat_level == VAT_600:
            product.customer_vat = product.customer_unit_price - (product.customer_unit_price / DECIMAL_1_21).quantize(
                FOUR_DECIMALS)
        product.compensation = DECIMAL_ZERO
        if product.vat_level == VAT_200:
            product.compensation = (product.customer_unit_price * DECIMAL_0_02).quantize(FOUR_DECIMALS)
        ...
        if self.id is not None:
            if len(self.producers.all()) > 0:
                if self.status == PERMANENCE_PLANNED:
                    ...
                    link = []
                    for p in self.producers.filter(is_active=True).only("id", "short_profile_name",
                                                                        "invoice_by_basket"):
                        ...
                        if pi is not None:
                            link.append('<a href="' + changelist_url +
                                        ...
                                        '&producer=' + str(p.id) +
                                        '" target="_blank" class="addlink">' +
                                        p.short_profile_name.replace(' ', '&nbsp;') + '&nbsp;(' + number_format(
                                pi.total_price_with_tax, 2).replace(' ', '&nbsp;') + '&nbsp;&euro;)</a>'
                                        )
                        else:
                            link.append('<a href="' + changelist_url +
                                        '?permanence=' + str(self.id) +
                    ...
                    producers = "<button onclick=\"django.jQuery('#id_get_producers_%d').toggle();" \
                                ...
                                "django.jQuery(this).html('%s')" \
                                "};" \
                                "return false;\">%s</button><div id=\"id_get_producers_%d\" style=\"display:none;\">" % (
...
class OfferItem(TranslatableModel):
        ...
        if self.manage_stock:
            if (self.previous_add_2_stock != self.add_2_stock or
                ...
                previous_purchase_price = ((self.previous_producer_unit_price +
                                            self.previous_unit_deposit) * self.previous_add_2_stock).quantize(
                    TWO_DECIMALS)
    ...
    def reference_price_with_vat(self):
        if self.order_average_weight > DECIMAL_ZERO:
            reference_price = ((DECIMAL_ONE / self.order_average_weight) * self.customer_unit_price).quantize(
                TWO_DECIMALS)
            if self.order_unit in [PRODUCT_ORDER_UNIT_PC_PRICE_KG, PRODUCT_ORDER_UNIT_PC_PRICE_LT,
                                   PRODUCT_ORDER_UNIT_PC_PRICE_PC]:
                return number_format(reference_price, 2)
