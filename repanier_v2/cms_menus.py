...
from repanier_v2.models import PermanenceBoard
...
class PermanenceMenu(Menu):
    ...
    def append_permanence_board(self, nodes, parent_node, submenu_id):
        permanence_board_set = (
            PermanenceBoard.objects.filter(
                permanence__status__lte=ORDER_WAIT_FOR_INVOICED
            )
            .only("id")
            .order_by("?")
        )
        ...
        if permanence_board_set.exists():
            ...
            node = NavigationNode(
                ...
                reverse("repanier_v2:permanence_view"),
                id=submenu_id,
    ...
    def append_permanence(
        ...
        path = reverse("repanier_v2:order_view", args=(permanence.id,))
        if not is_anonymous and permanence.status > ORDER_OPENED:
