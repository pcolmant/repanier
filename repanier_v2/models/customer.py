...
class Customer(models.Model):
    ...
    def get_balance(self):
        ...
        if last_customer_invoice.exists():
            if balance.amount >= 30:
                return format_html(
                    '<a href="{}" class="repanier_v2-a-info" target="_blank" ><span style="color:#32CD32">{}</span></a>',
                    reverse("repanier_v2:customer_invoice_view", args=(0, self.id)),
                    balance,
                )
            elif balance.amount >= -10:
                return format_html(
                    '<a href="{}" class="repanier_v2-a-info" target="_blank" ><span style="color:#696969">{}</span></a>',
                    reverse("repanier_v2:customer_invoice_view", args=(0, self.id)),
                    balance,
                )
            else:
                return format_html(
                    '<a href="{}" class="repanier_v2-a-info" target="_blank" ><span style="color:red">{}</span></a>',
                    reverse("repanier_v2:customer_invoice_view", args=(0, self.id)),
                    balance,
                )
        else:
    ...
    def anonymize(self, also_group=False):
        ...
        self.user.username = self.user.email = "{}@repanier_v2.be".format(self.short_name)
    ...
    def get_absolute_url(self):
        ...
        return reverse(
            "repanier_v2:published_customer_view", kwargs={"customer_id": self.id}
        )
