...
from repanier_v2.tools import cap, create_or_update_one_purchase
...
class Permanence(TranslatableModel):
                ...
                if pi.status == ORDER_SEND:
                    ...
                    link.append(
                        '<a href="{}?producer={}" target="_blank">{}</a>'.format(
                            reverse("repanier_v2:producer_invoice_view", args=(pi.id,)),
                            pi.producer_id,
                            label.replace(" ", "&nbsp;"),
                        )
                    )
            ...
            for ci in (
                ...
                link.append(
                    '<a href="{}?customer={}" target="_blank">{}</a>'.format(
                        reverse("repanier_v2:customer_invoice_view", args=(ci.id,)),
                        ci.customer_id,
                        label.replace(" ", "&nbsp;"),
                    )
                )
    ...
    def invoice(self, payment_date):
        ...
        self.set_status(
            old_status=ORDER_SEND, new_status=ORDER_WAIT_FOR_INVOICED
        )

        customer_invoice_buyinggroup = self.get_or_create_group_invoice(
            customer_buyinggroup, payment_date, ORDER_WAIT_FOR_INVOICED
        )
        ...
        if permanence_partially_invoiced:
            ...
            new_permanence = self.create_child(ORDER_SEND)
            new_permanence.producers.add(*producers_to_move)
            ProducerInvoice.objects.filter(
                permanence_id=self.id, producer_id__in=producers_to_move
            ).update(
                permanence_id=new_permanence.id, status=ORDER_SEND
            )
            CustomerProducerInvoice.objects.filter(
                permanence_id=self.id,
    ...
    def cancel_invoice(self, last_bank_account_total):
        ...
        self.set_status(
            old_status=ORDER_WAIT_FOR_CANCEL_INVOICE, new_status=ORDER_SEND
        )
    ...
    def recalculate_order_amount(
        ...
        for a_purchase in purchase_set.select_related("offer_item", "customer_invoice"):
            ...
            if send_to_producer or re_init:
                ...
                if send_to_producer and a_purchase.status == ORDER_WAIT_FOR_SEND:
