...
from repanier_v2.packages.openpyxl.writer.excel import save_virtual_workbook
from repanier_v2.tools import *
from repanier_v2.xlsx.xlsx_order import generate_customer_xlsx, generate_producer_xlsx
        ...
        context = TemplateContext(
            {
                ...
                "last_balance_link": mark_safe(
                    '<a href="https://{}{}">{}</a>'.format(
                        settings.ALLOWED_HOSTS[0],
                        reverse("repanier_v2:customer_invoice_view", args=(0, 0)),
                        _("Group invoices"),
                    )
                ),
...
def export_order_2_1_customer(
    ...
    if customer_invoice is not None:
        if order_responsible is None:
            order_responsible = Staff.get_or_create_order_responsible()
        wb = generate_customer_xlsx(
            permanence=permanence, customer_invoice=customer_invoice
        )[0]
        if wb is not None:
