...
from repanier_v2.admin.admin_filter import (
    ...
    FrozenItemFilter,
    FrozenItemSendFilterByPermanence,
)
from repanier_v2.admin.inline_foreign_key_cache_mixin import InlineForeignKeyCacheMixin
from repanier_v2.const import *
from repanier_v2.fields.RepanierMoneyField import FormMoneyField
from repanier_v2.models import LUT_DepartmentForCustomer
from repanier_v2.models.customer import Customer
from repanier_v2.models.frozen_item import FrozenItem
...
from repanier_v2.tools import rule_of_3_reload_purchase


class FrozenItemPurchaseSendInlineFormSet(BaseInlineFormSet):
    def clean(self):
        if any(self.errors):
            ...
            if form.cleaned_data and not form.cleaned_data.get("DELETE"):
                ...
                if value is not None:
                    if value in values:
                        ...
                        values.add(value)
                    qty += form.cleaned_data.get("qty", DECIMAL_ZERO).quantize(
                        THREE_DECIMALS
                    )


class FrozenItemPurchaseSendInlineForm(forms.ModelForm):
...
class FrozenItemPurchaseSendInline(InlineForeignKeyCacheMixin, admin.TabularInline):
    form = FrozenItemPurchaseSendInlineForm
    formset = FrozenItemPurchaseSendInlineFormSet
    model = Purchase
    fields = ["customer", "qty", "purchase_price", "comment"]
    extra = 0
    fk_name = "frozen_item"
...
class FrozenItemSendDataForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["offer_purchase_price"].initial = offer_item.at_producer_tariff
        ...
        if offer_item.producer_tariff_is_wo_tax:
...
class FrozenItemSendAdmin(admin.ModelAdmin):
    form = FrozenItemSendDataForm
    inlines = [FrozenItemPurchaseSendInline]
    ...
    def get_form(self, request, obj=None, **kwargs):
        ...
        if obj is not None:
            ...
            department_field.queryset = LUT_DepartmentForCustomer.objects.none()
    ...
    def save_related(self, request, form, formsets, change):
        ...
        offer_item = FrozenItem.objects.filter(id=form.instance.id).order_by("?").first()
        formset = formsets[0]
            ...
            if customer is None:
                ...
                if purchase_price != previous_purchase_price:
                    ...
                    purchase.purchase_price.amount = (
                        purchase.qty * offer_item.producer_unit_price.amount
        ...
        offer_item.permanence.recalculate_order_amount(
            offer_item_qs=FrozenItem.objects.filter(id=offer_item.id).order_by("?")
        )
