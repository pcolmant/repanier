...
from repanier_v2.admin.admin_model import RepanierAdmin
...
from repanier_v2.models.lut import LUT_PermanenceRole, LUT_DeliveryPoint
from repanier_v2.models.order import OrderInPreparation
from repanier_v2.models.order_dispensing_point import OrderDispensingPoint
...
from repanier_v2.task import task_order
from repanier_v2.task.task_order import open_order, close_order
from repanier_v2.tools import get_recurrence_dates, get_repanier_template_name
from repanier_v2.xlsx.xlsx_offer import export_offer
from repanier_v2.xlsx.xlsx_order import generate_producer_xlsx, generate_customer_xlsx

logger = logging.getLogger(__name__)


class OrderInPreparationInlineMixin(InlineForeignKeyCacheMixin):
    ...
    def has_delete_permission(self, request, obj=None):
        if self._has_add_or_delete_permission is None:
            ...
            if object_id:
                # Update
                parent_object = (
                    OrderInPreparation.objects.filter(id=object_id)
                    .only("status")
                    .first()
                )
                if parent_object is not None and parent_object.status == ORDER_PLANNED:
...
class OrderTaskInline(OrderInPreparationInlineMixin, admin.TabularInline):
    model = OrderTask
    ordering = [
...
class OrderDistributionPointInline(OrderInPreparationInlineMixin, admin.TabularInline):
    model = OrderDispensingPoint
    fields = ["delivery_comment", "delivery_point", "order_is_open"]
    extra = 0
    readonly_fields = ["order_is_open"]
...
class OrderInPreparationForm(forms.ModelForm):
    ...
    class Meta:
        model = OrderInPreparation
        fields = "__all__"


class OrderInPreparationAdmin(RepanierAdmin):
    form = OrderInPreparationForm
    change_list_url = reverse_lazy("admin:repanier_permanenceinpreparation_changelist")

    list_per_page = 20
    list_max_show_all = 10
    filter_horizontal = ("producers", "boxes")
    inlines = [OrderDistributionPointInline, OrderTaskInline]
    date_hierarchy = "order_date"
    list_display = ("get_order_admin_display",)
    list_display_links = ("get_order_admin_display",)
    ...
    def get_readonly_fields(self, request, permanence=None):
        if permanence is not None and permanence.status > ORDER_PLANNED:
    ...
    def get_formsets_with_inlines(self, request, obj=None):
        for inline in self.get_inline_instances(request, obj):
            # hide OrderDistributionPointInline if no delivery point
            if (
                isinstance(inline, OrderDistributionPointInline)
                and not LUT_DeliveryPoint.objects.filter(is_active=True).exists()
            ):
                continue
            # hide OrderDistributionPointInline if no permanence role
            if (
                isinstance(inline, OrderTaskInline)
    ...
    @check_permanence(ORDER_PLANNED, ORDER_PLANNED_STR)
    ...
    @check_cancel_in_post
    @check_permanence(ORDER_OPENED, ORDER_OPENED_STR)
        ...
        return render(
            ...
            template_name,
            {
                **self.admin_site.each_context(request),
                ...
                "deliveries": OrderDispensingPoint.objects.filter(permanence_id=permanence.id),
            },
        )
    ...
    def open_order(self, request, permanence_id, permanence=None):
        ...
        if email_will_be_sent:
            ...
            template = Template(
                repanier_v2.globals.REPANIER_SETTINGS_CONFIG.offer_customer_mail
            )
            ...
            offer_detail = "<ul>{}</ul>".format(
                "".join(
                    "<li>{}, {}</li>".format(p.get_long_name(), p.producer.short_name)
                    for p in qs
                )
            )
            ...
            if settings.REPANIER_SETTINGS_CUSTOMER_MUST_CONFIRM_ORDER:
                template = Template(
                    repanier_v2.globals.REPANIER_SETTINGS_CONFIG.cancel_order_customer_mail
                )
    ...
    def close_order(self, request, permanence_id, permanence=None):
        ...
        (
            ...
            is_email_send=repanier_v2.globals.REPANIER_SETTINGS_SEND_ORDER_MAIL_TO_BOARD,
            board=True,
        )
        ...
        if email_will_be_sent:
            ...
            if order_customer_email_will_be_sent:
                template = Template(
                    repanier_v2.globals.REPANIER_SETTINGS_CONFIG.order_customer_mail
                )
                ...
                bank_account_number = repanier_v2.globals.REPANIER_SETTINGS_BANK_ACCOUNT
            ...
            if order_producer_email_will_be_sent:
                template = Template(
                    repanier_v2.globals.REPANIER_SETTINGS_CONFIG.order_producer_mail
                )
                context = TemplateContext(
                    {
                        "name": _("Long name"),
            ...
            if order_board_email_will_be_sent:
                ...
                template = Template(
                    repanier_v2.globals.REPANIER_SETTINGS_CONFIG.order_staff_mail
                )
                context = TemplateContext(
                    {
                        "permanence_link": format_html(
        ...
        if permanence.with_delivery_point:
            deliveries = OrderDispensingPoint.objects.filter(
                permanence_id=permanence.id,
                status__in=[ORDER_OPENED, ORDER_CLOSED],
            )
        else:
            deliveries = OrderDispensingPoint.objects.none()
        ...
        return render(
            ...
            template_name,
            {
                **self.admin_site.each_context(request),
                ...
                "permanence": permanence,
                "permanenceboard": OrderTask.objects.filter(
                    permanence=permanence_id
                ).order_by("permanence_role"),
                "deliverypoint": OrderDispensingPoint.objects.filter(permanence=permanence_id),
                "form": form,
                "action_checkbox_name": helpers.ACTION_CHECKBOX_NAME,
            },
        )
    ...
    def get_row_actions(self, permanence):
        ...
        if permanence.status == ORDER_PLANNED:
            return format_html(
                '<div class="repanier_v2-button-row">'
                '<span class="repanier_v2-a-container"><a class="repanier_v2-a-tooltip repanier_v2-a-info" href="{}" data-repanier_v2-tooltip="{}"><i class="fas fa-retweet"></i></a></span>'
                '<span class="repanier_v2-a-container"><a class="repanier_v2-a-tooltip repanier_v2-a-info" href="{}" data-repanier_v2-tooltip="{}"><i class="fas fa-play" style="color: #32CD32;"></i></a></span>'
                "</div>",
        ...
        elif permanence.status == ORDER_OPENED:
            return format_html(
                '<div class="repanier_v2-button-row">'
                '<span class="repanier_v2-a-container"><a class="repanier_v2-a-tooltip repanier_v2-a-info" href="{}" data-repanier_v2-tooltip="{}"><i class="fas fa-pencil-alt"></i></a></span>'
                '<span class="repanier_v2-a-container"><a class="repanier_v2-a-tooltip repanier_v2-a-info" href="{}" data-repanier_v2-tooltip="{}"><i class="fas fa-stop" style="color: red;"></i></a></span>'
                "</div>",
                add_filter(
    ...
    def get_queryset(self, request):
        ...
        return qs.filter(status__lte=ORDER_SEND)

    @transaction.atomic
    def save_related(self, request, form, formsets, change):
        super().save_related(request, form, formsets, change)
        permanence = form.instance
        permanence.with_delivery_point = OrderDispensingPoint.objects.filter(
            permanence_id=permanence.id
        ).exists()
    ...
    def save_model(self, request, permanence, form, change):
        if change and ("permanence_date" in form.changed_data):
            OrderTask.objects.filter(permanence_id=permanence.id).update(
