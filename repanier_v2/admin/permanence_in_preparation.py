...
from repanier_v2.admin.tools import (
    ...
    check_permanence,)
from repanier_v2.middleware import (
    get_query_filters,
    add_filter,
)
        ...
        if self._has_add_or_delete_permission is None:
            ...
            if object_id:
                ...
                if (
                    ...
                    and parent_object.status == ORDER_PLANNED
                ):
...
class DeliveryBoardInline(PermanenceInPreparationInlineMixin, TranslatableTabularInline):
    model = DeliveryBoard
    ordering = ("id",)
...
class PermanenceInPreparationAdmin(TranslatableAdmin):
        ...
        if email_will_be_sent:
            ...
            for language in settings.PARLER_LANGUAGES[settings.SITE_ID]:
                ...
                with switch_language(
                    repanier_v2.apps.REPANIER_SETTINGS_CONFIG, language_code
                ):
                    template = Template(
                        repanier_v2.apps.REPANIER_SETTINGS_CONFIG.offer_customer_mail
                    )
                ...
                if settings.REPANIER_SETTINGS_CUSTOMER_MUST_CONFIRM_ORDER:
                    with switch_language(
                        ...
                        template = Template(
                            repanier_v2.apps.REPANIER_SETTINGS_CONFIG.cancel_order_customer_mail
                        )
    ...
    def close_order(self, request, permanence_id, permanence=None):
        ...
        order_board_email_will_be_sent, order_board_email_will_be_sent_to = RepanierEmail.send_email_to_who(
            is_email_send=repanier_v2.apps.REPANIER_SETTINGS_SEND_ORDER_MAIL_TO_BOARD,
            board=True,
        )
        ...
        if email_will_be_sent:
            ...
            for language in settings.PARLER_LANGUAGES[settings.SITE_ID]:
                ...
                if order_customer_email_will_be_sent:
                    template = Template(
                        repanier_v2.apps.REPANIER_SETTINGS_CONFIG.order_customer_mail
                    )
                    ...
                    bank_account_number = repanier_v2.apps.REPANIER_SETTINGS_BANK_ACCOUNT
                ...
                if order_producer_email_will_be_sent:
                    template = Template(
                        repanier_v2.apps.REPANIER_SETTINGS_CONFIG.order_producer_mail
                    )
                    context = TemplateContext(
                        {
                            "name": _("Long name"),
                ...
                if order_board_email_will_be_sent:
                    ...
                    template = Template(
                        repanier_v2.apps.REPANIER_SETTINGS_CONFIG.order_staff_mail
                    )
                    context = TemplateContext(
                        {
                            "permanence_link": format_html(
